...
const PillNav = ({
  ...
  useEffect(() => {
    ...
    if (initialLoadAnimation && !hasPlayedInitialLoadRef.current) {
      ...
      if (navItems) {
        ...
        gsap.to(navItems, {
          ...
          ease,
          onComplete: () => {
            // Width reveal animation bittikten sonra dropdown'larin kirpilmamasi icin overflow'u geri ac.
        ...
        {showLogo && (
          ...
          >
            {logo ? (
              ...
              <span className="logo-fallback">C</span>
            )}
          </a>
        )}
            ...
            {items.map((item, i) => (
              ...
              >
                <a
                  ...
                  aria-haspopup={Array.isArray(item.links) && item.links.length ? "menu" : undefined}
                ...
                {Array.isArray(item.links) && item.links.length > 0 && (
                  <div
                    className={`pill-dropdown ${openDropdownIndex === i ? "open" : ""}`}
                    role="menu"
                    aria-label={`${item.label} menusu`}
                  >
                    {item.links.map((link, linkIndex) => (
                      <a
                        key={`${item.label}-link-${linkIndex}`}
                        href={link.href || "#"}
                        className="pill-dropdown-link"
                        role="menuitem"
        ...
        <ul className="mobile-menu-list">
          {items.map((item, i) => (
            <li key={item.href || `mobile-item-${i}`}>
              <a href={item.href} className={`mobile-menu-link${activeHref === item.href ? " is-active" : ""}`}>
                {item.label}
              </a>
            </li>
          ))}
